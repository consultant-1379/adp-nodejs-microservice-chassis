...
const mapSuite = (suite, testFile, errors, title = '') => {
  ...
  const tests = suite.tests.map(test =>
    mapTest(test, uuid, errors, title + suite.name),
  );
  const suites = suite.suites.map(s =>
    mapSuite(s, testFile, errors, title + suite.name),
  );
...
class ConfigMapper {
  ...
  collectTestData() {
    this.browsers.forEach(browser => {
      ...
      const testData = this.sessions.filter(
        session => session.browser.name === browser,
      );
  ...
  generateStats(startTime, endTime) {
    ...
    this.browsers.forEach(browser => {
      ...
      this.stats[browser] = {
        ...
        passPercent:
          ((sums.numberOfTests - sums.numberOfFailures) / sums.numberOfTests) *
          100,
        pendingPercent: 0,
        other: 0,
        hasOther: false,
